...
│   ├── uhmwebmin.sh              # Webmin module installer/uninstaller — real-time log viewer
...
| `/etc/uhm/uhm.env` | Configuration (interfaces, IPs, credentials, ports) | Configuración |
...
| `PYDHCPD_LEASES` | pydhcpd's own leases file path, read from `pydhcp.env` at runtime; read by `uhmd.sh` and `uhmleases.sh` (default `/etc/pydhcp/pydhcpd.leases`) | Ruta del archivo de leases de pydhcpd, leída desde `pydhcp.env` en cada ejecución; usada por `uhmd.sh` y `uhmleases.sh` (default `/etc/pydhcp/pydhcpd.leases`) |
...
| **Level badges** | Color-coded badges, one distinctive color per level: INFO (blue), WARNING (amber), ERROR (red), FIX (green), ALERT (purple), STATUS (grey). | Badges con color, un color distintivo por nivel: INFO (azul), WARNING (ámbar), ERROR (rojo), FIX (verde), ALERT (púrpura), STATUS (gris). |
...
| Path | Role | Rol |
|---|---|---|
| `/etc/acl/acl_mac/mac-limited.txt` | Authorized — forced through Squid | Autorizados — forzados por Squid |
...
=== 02:00:00:aa:bb:99 ===
 ...
 blockdhcp.txt:    N
 acl_mac/*.txt:    Y
        /etc/acl/acl_mac/mac-limited.txt
 pydhcpd.leases:   N
